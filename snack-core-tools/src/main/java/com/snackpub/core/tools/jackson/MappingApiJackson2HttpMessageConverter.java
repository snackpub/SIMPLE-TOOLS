...
/**
 * 针对api服务对 android和ios和web处理的 分读写的jackson的处理
 *
 * <p>
 * 1. app端上报数据是使用 readObjectMapper
 * 2. 返回给app端的数据使用 writeObjectMapper
 * </p>
 *
 * @author L.cm
 */
...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
    ...
    private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper) {
        ...
        // null 处理
        writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new RisesinBeanSerializerModifier()));
        writeObjectMapper.getSerializerProvider().setNullValueSerializer(RisesinBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
